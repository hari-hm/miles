"""Minimal framed, multiplexed RPC over a connected stream socket.

Used between the session router (client side: :meth:`IpcChannel.request`) and a
session worker (server side: a ``request_handler`` coroutine). One channel wraps
one socket (a ``socket.socketpair`` end). Concurrent requests are multiplexed by
a ``request_id`` so small replies do not wait behind earlier ones.

Wire format (one whole message per frame, no chunking):

    u32 length  |  u64 request_id  |  u8 type  |  payload bytes
    └ length counts everything after it (the 9-byte header + payload) ┘

``type`` is REQUEST / REPLY / ERROR. The ``payload`` is opaque to the channel;
the worker/router layer encodes it with :func:`encode_envelope`. A REPLY carries
the handler's return bytes; an ERROR carries a UTF-8 error message.

Deliberately minimal: no chunking, no round-robin scheduling, no send-buffer
budget, no configurable size limits — a single ``_MAX_FRAME`` sanity cap guards
against a corrupt length triggering a huge allocation. Large bodies (e.g. a
``GET /sessions`` records dump) are sent as one frame.
"""
...
# corrupt/garbage u32 length cannot trigger a multi-GiB readexactly. Generous vs
# the largest real body (retained-records GET, ~hundreds of MiB).
_MAX_FRAME = 2 * 1024 * 1024 * 1024  # 2 GiB
