"""Supervisor for the multi-process session server.

Runs in the rollout process. Spawns N worker processes + 1 router process, wires a
``socket.socketpair`` per worker, and waits until the router reports all workers
healthy. Then it fail-fast monitors the group: if any child dies it records the
failure (surfaced to the rollout path via :meth:`check`) and tears the whole group
down — SIGTERM, a short grace period, then SIGKILL — so no orphan processes survive.

Multi-process is opt-in (``--session-server-workers > 1``); ``= 1`` never builds a
supervisor and keeps the single-process server.
"""
