"""Headless session worker process (multi-process data plane).

A worker owns one shard of sessions: its own ``SessionRegistry`` (via
``SessionCore``) plus an httpx ``ProxyBackend`` to the inference backend. It
speaks IPC, not HTTP: it decodes a request envelope, drives the matching
``SessionCore`` method, and serializes the returned Starlette ``Response``
(status / headers / body bytes) back over the channel.

The router routes by ``blake2b(session_id) % N``; the worker trusts that routing
and simply handles whatever it is told (no ownership re-derivation).
"""
...
from miles.rollout.session.session_core import ProxyRequest, build_session_core, error_response
from miles.rollout.session.session_errors import SessionError
