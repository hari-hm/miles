...
Thin layer: converts each HTTP request to primitive inputs, calls the
HTTP ``SessionCore``. All session/TITO logic lives in ``session_core``.
"""
...
from miles.rollout.session.session_core import build_session_core, error_response
