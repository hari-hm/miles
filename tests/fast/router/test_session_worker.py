...
from miles.rollout.session.session_core import build_session_core
...
from miles.rollout.session.session_worker import SessionWorker
...
async def test_get_missing_session_returns_404(worker):
    status, _, body = _reply(await worker.handle(encode_request(OP_GET, session_id=new_session_id())))
    assert status == 404
    assert "not found" in json.loads(body)["error"]
...
async def test_proxy_passthrough(worker):
    status, headers, body = _reply(
        await worker.handle(encode_request(OP_PROXY, session_id=new_session_id(), path="v1/models", method="GET"))
    )
    assert status == 200
